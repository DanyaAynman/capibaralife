...
let timer=null,tick=null,timeoutRequested=false,phase='lobby',topics=[],turn='',choice=null,wagers=new Map,finalAns=new Map;
...
function handle(d){if(d.type==='hello'&&host){if(!players.has(d.id)&&players.size<8)players.set(d.id,player(d.id,(d.name||'Игрок').slice(0,18),d.profile||{a:0,c:0}));renderPlayers();sync(d.id);return}if(d.target&&d.target!==me)return;if(host&&d.type==='timeout_request'){if(open&&current&&Date.now()-current.startedAt>=29500)closeQ();return}if(d.type==='state'){players=new Map((d.players||[]).map(p=>[p.id,p]));chooser=d.chooser;round=d.round;used=new Set(d.used||[]);current=d.current;open=d.open;phase=d.phase;topics=d.topics||[];turn=d.turn||'';choice=d.choice;render();return}if(host&&d.type==='select')return d.id===chooser&&!open&&selectQ(d.ci,d.ri);if(host&&d.type==='answer')return hostAnswer(d.id,d.answer);if(host&&d.type==='remove_topic')return removeTopic(d.id,d.idx);if(host&&d.type==='wager')return hostWager(d.id,d.wager);if(host&&d.type==='final_answer')return hostFinal(d.id,d.answer);if(d.type==='game_start'){round=1;used.clear();chooser=d.chooser;phase='round';show('play');renderBoard()}if(d.type==='question'){current=d.current;open=true;timeoutRequested=false;submitted.clear();showQuestion()}if(d.type==='feedback'&&d.id===me){$('answerMsg').textContent=d.correct?'Правильно! +'+d.value:'Неверно. −'+d.value+' · Ждём ответ других игроков…';$('answerMsg').className='msg '+(d.correct?'ok':'bad')}if(d.type==='closed'){current=d.current;open=false;chooser=d.chooser;players=new Map(d.players.map(p=>[p.id,p]));renderPlayers();showClosed()}if(d.type==='next'){used=new Set(d.used);chooser=d.chooser;current=null;open=false;renderBoard()}if(d.type==='transition'){players=new Map(d.players.map(p=>[p.id,p]));transition()}if(d.type==='round2'){round=2;used.clear();chooser=d.chooser;phase='round';current=null;open=false;show('play');renderBoard()}if(d.type==='topics'){players=new Map(d.players.map(p=>[p.id,p]));topics=d.topics;turn=d.turn;phase='topics';showTopics()}if(d.type==='topic_update'){topics=d.topics;turn=d.turn;showTopics()}if(d.type==='topic_chosen'){choice=d.choice;phase='wager';showWager()}if(d.type==='final_q'){choice=d.choice;phase='finalq';showFinalQ()}if(d.type==='results'){players=new Map(d.players.map(p=>[p.id,p]));showResults(d.results)}}
...
function showQuestion(){show('play');$('boardWrap').classList.add('hidden');$('questionWrap').classList.remove('hidden');$('qMeta').textContent=current.cat+' · '+current.value;$('qText').textContent=current.question;$('qVisual').innerHTML=media(current);$('qVisual').classList.toggle('hidden',!current.media);$('answerInput').value='';$('answerInput').disabled=false;$('answerBtn').disabled=false;$('answerMsg').textContent='';$('answerReveal').classList.add('hidden');clearInterval(tick);const update=()=>{if(!current)return;const left=Math.max(0,30-Math.floor((Date.now()-current.startedAt)/1000));$('qTimer').textContent=left;if(left<=0&&open){if(host)closeQ();else if(!timeoutRequested){timeoutRequested=true;bc('timeout_request',{id:me})}}};update();tick=setInterval(update,250)}
...
function hostAnswer(id,a){if(!open||answered.has(id)||!players.has(id))return;answered.add(id);const good=ok(a,current.answers),p=players.get(id);p.score+=good?current.value:-current.value;bc('feedback',{id,correct:good,value:current.value});if(id===me&&!good){$('answerMsg').textContent='Неверно. −'+current.value+' · Ждём ответ других игроков…';$('answerMsg').className='msg bad'}if(good){chooser=id;open=false;clearTimeout(timer);clearInterval(tick);current={...current,result:'correct',winnerId:id,winnerName:p.name};bc('closed',{current,chooser,players:snap()});showClosed();nextLater()}else if(answered.size>=players.size)closeQ()}
function closeQ(){if(!host||!open)return;open=false;clearTimeout(timer);clearInterval(tick);chooser=current.prev;current={...current,result:'timeout'};bc('closed',{current,chooser,players:snap()});showClosed();nextLater()}
function showClosed(){clearInterval(tick);$('qTimer').textContent='0';$('answerInput').disabled=true;$('answerBtn').disabled=true;$('answerMsg').textContent=current.result==='correct'?(current.winnerId===me?'Правильно! +'+current.value:current.winnerName+' ответил правильно'):'Время вышло';$('answerMsg').className='msg '+(current.result==='correct'?'ok':'');$('answerReveal').textContent='Правильный ответ: '+current.answers[0];$('answerReveal').classList.remove('hidden')}
function nextLater(){setTimeout(()=>{if(used.size===25)return roundEnd();current=null;open=false;bc('next',{chooser,used:[...used]});renderBoard()},3000)}
